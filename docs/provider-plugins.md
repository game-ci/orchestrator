...
> **Standalone package**: These features are part of [`@game-ci/orchestrator`](https://github.com/game-ci/orchestrator). When installed, unity-builder automatically loads them via the plugin interface.
...
**Via GitHub Action (unity-builder plugin):**
```yaml
...
**Via CLI:**
```bash
npx game-ci orchestrate --providerStrategy local --providerExecutable ./my-provider
```
