...
export class BuildReliabilityService {
  ...
  static clearUnityLibrarySubfolders(projectPath: string, subfolders: string[]): string[] {
    const libraryPath = path.join(projectPath, 'Library');
    const removed: string[] = [];

    for (const subfolder of subfolders) {
      const target = path.join(libraryPath, subfolder);
      if (BuildReliabilityService.removeDirectoryWithRetry(target)) {
        removed.push(target);
      }
    }
  ...
  static removeAutoGeneratedAssets(projectPath: string, relativeAssetPaths: string[]): string[] {
    const removed: string[] = [];
  ...
  static removeDirectoryWithRetry(directoryPath: string, attempts = 4, delayMs = 500): boolean {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        ...
        if (!fs.existsSync(directoryPath)) {
          core.info(`[Reliability] Removed directory: ${directoryPath}`);

          return true;
        }
      } catch (error: any) {
        if (attempt === attempts) {
          core.warning(`[Reliability] Failed to remove ${directoryPath}: ${error.message}`);
        }
      }

      if (attempt < attempts && delayMs > 0) {
        BuildReliabilityService.sleepSync(delayMs);
      }
    }
  ...
  private static sleepSync(delayMs: number): void {
